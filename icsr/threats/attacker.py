...
class ICS_Attacker:
    ...
    def __init__(self, cfg: dict, logger: log.Log) -> None:
        """
        CTOR
        Args:
            cfg: configuration object for attack generation
        ...
        ThreatFactory.init(cfg, logger)

    @staticmethod
    def physics_law(a_data: pd.DataFrame,
                    cycle,
                    i,
                    logger) -> pd.DataFrame:
        """
        Method injects attack in accordance to physics based control law provided by the
        ICS model definition.
        Args:
            a_data: input data that will be modified to generate the attack
            cycle: cycle length
            i: current iteration
            logger: logger object
        Returns:
            <pd.DataFrame>: returned data set with attack modified SW
        """
        ...
        pid = AutoclaveProcess.PIDController(cycle, logger)
        ...
        # Remove PID controller from memory
        pid = None
        gc.collect()
    ...
    def attack(self,
        ...
        def __gen_attack_table(prob: float, num_cycles: int) -> list:
            ...
            h_attack_cycles = int(t_attack_cycles / 2)

            h_non_attack_cycles = half_cycle - h_attack_cycles

            attack_list_1 = np.concatenate(
                (
                    np.zeros(h_non_attack_cycles, dtype=bool),
                    np.ones(h_attack_cycles, dtype=bool),
                )
            ).tolist()
        ...
        for i in range(0, len(a_data.index), cycle):
            ...
            # self.log.debug(f"Attack |{cycle_idx}| => |{attack_type}|")
            ...
            for acfg in config["sets"]:
                ...
                a_data.loc[i: i + cycle, ["tc", "type", "label"]] = ThreatFactory.run(
                    ThreatClass[f"{config['name'].upper()}"],
                    config,
            ...
            if "physics" == config["control_law"]:
                a_data = ICS_Attacker.physics_law(a_data,
                                                  cycle,
                                                  i,
                                                  self.log)
            else:
                a_data.loc[:, "temp"] = a_data.loc[:, "tc"]
            ...
            if not attack_samples[attack_type_val]:
                if "file" in cfg["attack"].keys():
                    ...
                    Plotter.input_ts_color(
                        data.loc[a_data.loc[i, "time"]: a_data.loc[i + cycle, "time"], ],

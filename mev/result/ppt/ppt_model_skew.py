...
class ModelSkewPresentation(PresentationBase):
    """
    ...
    def results_phase_1(self, meta_images: dict) -> None:
        ...
        for images in meta_images.values():
            self.prs.create_slide_image("Model skew", images, [6.42, 9.37], False)
    ...
    def run(self, data: pd.DataFrame, file_name: str) -> None:
        ...
        # X - Baseline and skewed, Y - AUC performance
        self.results_phase_1(images)

        # X - Baseline and skewed, Y - Accuracy
        # self.results_phase_2()

        # X - Baseline and skewed, Y - F1 score
        # self.results_phase_3()

        # Saving presentation file
        self.prs.save(file_name)
        self.log.debug(f"Model skew summary PPT saved to: {file_name}")
